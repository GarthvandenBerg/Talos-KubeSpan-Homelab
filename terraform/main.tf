...
data "talos_machine_configuration" "controlplane_cloud" {
  ...
  config_patches     = [
    ...
    file("${path.module}/../.talos/patches/kubespan-patch.yaml"),
    yamlencode({
      machine = {
        install = {
...
resource "talos_machine_bootstrap" "bootstrap" {
  client_configuration = talos_machine_secrets.machine_secrets.client_configuration
  node                 = local.ctl_list[1].ip # only because I want to bootstrap the local ctl node
...
resource "null_resource" "talos_upgrade" {
  for_each = nonsensitive(local.all_nodes)

  triggers = {
    talos_image  = each.value.talos_image
    talos_config = sha256(data.talos_client_configuration.talosconfig.talos_config)
  }

  provisioner "local-exec" {
  interpreter = ["bash", "-c"]
  command     = <<EOF
    echo "🚀 Upgrading ${each.key} (${each.value.ip})"
    ...
    talosctl upgrade \
      ...
      --preserve=true \
      --wait=false \
      --talosconfig "${local_sensitive_file.talosconfig_temp.filename}"

    echo "💤 Waiting 60s for reboot and image pull..."
    sleep 60

    echo "🔍 Polling ${each.value.ip}..."
    timeout_counter=0
    
    while [ $timeout_counter -lt 30 ]; do
      if talosctl version -n ${each.value.ip} --short --talosconfig "${local_sensitive_file.talosconfig_temp.filename}" >/dev/null 2>&1; then
        echo -e "\n✅ ${each.key} upgraded and online!"
        exit 0
      fi
    ...
    echo -e "\n❌ Timeout reached for ${each.key}"
    exit 1
EOF
  }

  depends_on = [
    proxmox_virtual_environment_vm.vm_instance,
    talos_machine_configuration_apply.cp,
    talos_machine_configuration_apply.worker,
    local_sensitive_file.talosconfig_temp,
    time_sleep.wait_first_boot,
    helm_release.cilium
  ]
}
